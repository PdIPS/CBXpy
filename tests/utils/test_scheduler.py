...
def test_multiply_update():
    ...
    dyn = cbx.dynamic.CBO(f=lambda x: x**2, d=1, max_it=1, alpha=1.0, sigma=1.0)
...
def test_multiply_maximum():
    ...
    dyn = cbx.dynamic.CBO(f=lambda x: x**2, d=1, max_it=10, alpha=1.0, sigma=1.0)
...
def test_effective_number_scheduler():
    ...
    x = np.ones((6,5,7))
    dyn = cbx.dynamic.CBO(f=lambda x: np.sum(x**2), x=x, max_it=1, alpha=1.0, sigma=1.0)
